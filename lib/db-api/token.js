...
exports.createEmailValidationToken = function createEmailValidationToken(citizen, meta, fn) {
 ...
 /**
 * Validates a validation token does not exist for the provided IP address
 */
exports.validateIPUnique = function validateIPUnique(profile, meta, fn) {
  log('Validating that IP %s is not being used by other validation tokens', meta.ip);
  var query = Token.find({"meta.ip": meta.ip});

  query.exec(function(err, tokens) {
    if(tokens && tokens.length >= 2) {
      log('There are %s existing tokens with same IP as %s, requested by %s', tokens.length, meta.ip, profile.email);
      fn({message: t("login.ipLimit")});
    }
    fn(null);
    return this;
  });
