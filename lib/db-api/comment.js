...
/**
 * Guanyem Barcelona
 * Retrieve a list of current laws and number of comments,
 * to reflect the participation for a given law
 *
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 *   - 'replies', total comment replies or `undefined`
 ...
 * @api public
 */
exports.commentsByLaw = function commentsByLaw(fn) {
  log('Aggregating comments by law');

  Comment
    .aggregate(
      { $match: {context: "law"} },
      { $group: { _id: "$reference", total: { $sum: 1 } } },
      function (err, res) {
        if (err) {
          log('Found error: %j', err);
          return fn(err);
        ...
        log('%d laws with comments found', res.length);
        fn(null, res);
      }
    )
